...
# Dibujamos el brazo derecho
   glPushMatrix()
     glTranslatef(-(BODY_WIDTH)/2+0.5,(BODY_HEIGHT-ARM_HEIGHT)/2-0.5,0)
...
#Dibujamos la pierna izquierda
    ...
    glPopMatrix()
# ahora el píe
    glTranslatef(0.5,-(LEG_HEIGHT+LEG_WIDTH)/2,LEG_LENGTH)
    glColor3f(0.3,0.4,0.4)
    glScalef(LEG_WIDTH+1,LEG_WIDTH,LEG_LENGTH*2)
    glutSolidCube(1)
   glPopMatrix()
# Dibujamos la cabeza
   glColor3f(1,0.6,0.6)
